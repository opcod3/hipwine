...
void WINAPI wine___hipRegisterFunction(void **modules, const void *hostFunction,
                                ...
                                dim3 *blockDim, dim3 *gridDim, int *wSize)
{
    __hipRegisterFunction(modules, hostFunction, deviceFunction, deviceName, threadLimit, tid, bid, blockDim, gridDim, wSize);
...
hipError_t WINAPI wine_hipGetDevicePropertiesR0600(hipDeviceProp_tR0600 *prop, int deviceId)
{ 
    TRACE("()");
...
hipError_t WINAPI wine_hipDeviceGetName(char *name, int len, hipDevice_t device)
{
...
hipError_t WINAPI wine_hipCtxCreate(hipCtx_t *ctx, unsigned int flags, hipDevice_t device)
{
    TRACE("()");
...
hipError_t WINAPI wine_hipDeviceGetAttribute(int *pi, hipDeviceAttribute_t attr, int deviceId)
{
    TRACE("()\n");
...
hipError_t WINAPI wine_hipEventCreateWithFlags(hipEvent_t *event, unsigned int flags)
{
    TRACE("()\n");
...
hipError_t WINAPI wine_hipHostMalloc(void **ptr, size_t size, unsigned int flags)
{
...
hipError_t WINAPI wine_hipStreamCreateWithFlags(hipStream_t *stream, unsigned int flags)
{
    hipError_t err = hipStreamCreateWithFlags(stream, flags);
    HIPERR_ASSERT(err == 0);
    return err;
}
hipError_t WINAPI wine_hipMemcpyAsync(void *dst, const void *src, size_t sizeBytes, hipMemcpyKind kind,
                                      hipStream_t stream)
{
...
hipError_t WINAPI wine_hipMemcpy(void *dst, const void *src, size_t sizeBytes, hipMemcpyKind kind)
{
...
hipError_t WINAPI wine_hipMemGetInfo(size_t *free, size_t *total)
{
    ...
    HIPERR_ASSERT(err == 0);
    TRACE("\nmemGetInfo: free (%.3fmb), total (%.3fmb)", (float)(*free) / (1024.f * 1024.f), (float)(*total) / (1024.f * 1024.f));
    TRACE_RET();
    return err;
}

hipError_t WINAPI wine_hipHostGetDevicePointer(void **devPtr, void *hstPtr, unsigned int flags)
{
    TRACE("()");
...
hipError_t WINAPI wine_hipModuleLoadData(hipModule_t *module, const void *image)
{
    TRACE("()");
...
hipError_t WINAPI wine_hipModuleGetFunction(hipFunction_t *function, hipModule_t module, const char *kname)
{
    TRACE("()");
...
hipError_t WINAPI wine_hipFuncGetAttribute(int *value, hipFunction_attribute attrib, hipFunction_t hfunc)
{
    TRACE("()");
...
hipError_t WINAPI wine_hipFuncSetCacheConfig(const void *func, hipFuncCache_t config)
{
    TRACE("()");
...
hipError_t WINAPI wine_hipMemcpyHtoD(hipDeviceptr_t dst, void *src, size_t sizeBytes)
{
...
hipError_t WINAPI wine_hipMemsetD8Async(hipDeviceptr_t dest, unsigned char value, size_t count, hipStream_t stream)
{
...
hipError_t WINAPI wine_hipArrayCreate(hipArray_t *pHandle, const HIP_ARRAY_DESCRIPTOR *pAllocateArray)
{
...
hipError_t WINAPI wine_hipDestroyTextureObject(hipTextureObject_t textureObject)
{
...
hipError_t WINAPI wine_hipDeviceComputeCapability(int *major, int *minor, hipDevice_t device)
{
...
hipError_t WINAPI wine_hipModuleLaunchKernel(hipFunction_t f, unsigned int gridDimX, unsigned int gridDimY,
...
{
    TRACE("()");
    hipError_t err = hipModuleLaunchKernel(f, gridDimX, gridDimY, gridDimZ, blockDimX, blockDimY, blockDimZ, sharedMemBytes, stream, kernelParams, extra);
    assert(err == 0);
    TRACE_RET();
    return err;
};

hipError_t WINAPI wine_hipMemcpyDtoH(void *dst, hipDeviceptr_t src, size_t sizeBytes)
{
...
hipError_t WINAPI wine_hipMemcpyDtoDAsync(hipDeviceptr_t dst, hipDeviceptr_t src, size_t sizeBytes,
                                          hipStream_t stream)
{
...
hipError_t WINAPI wine_hipMemcpyDtoHAsync(void *dst, hipDeviceptr_t src, size_t sizeBytes, hipStream_t stream)
{
...
hipError_t WINAPI wine_hipStreamWaitEvent(hipStream_t stream, hipEvent_t event, unsigned int flags) {
...
hipError_t WINAPI wine_hipMemcpyParam2DAsync(const hip_Memcpy2D* pCopy, hipStream_t stream) {
...
{
    hipError_t err = hipTexObjectCreate(pTexObject, pResDesc, pTexDesc, pResViewDesc);
    HIPERR_ASSERT(err == 0);
    return err;
}

hipError_t WINAPI wine_hipMemcpyHtoDAsync(hipDeviceptr_t dst, void *src, size_t sizeBytes, hipStream_t stream)
{
...
hipError_t WINAPI wine_hipArray3DCreate(hipArray_t *array, const HIP_ARRAY3D_DESCRIPTOR *pAllocateArray)
{
...
hipChannelFormatDesc WINAPI wine_hipCreateChannelDesc(int x, int y, int z, int w, hipChannelFormatKind f)
{
